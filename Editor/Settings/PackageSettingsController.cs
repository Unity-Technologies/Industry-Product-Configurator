...
namespace IndustryCSE.Tool.ProductConfigurator.Settings.Editor
{
    public static class AssetRemoveConfirmation
    {
        ...
        public static void RemoveAssetConfirmation(VariantSetAsset variantSetAsset, List<VariantAsset> assets)
        {
            DelayAction = RemoveAll;
            switch (PackageSettingsController.Settings.RemoveBehaviour)
            {
                case RemoveBehaviour.AskEveryTime:
                    
                    EditorApplication.delayCall += DisplayDialog;
                    break;
                
                case RemoveBehaviour.RemoveWithoutAsking:
                    DelayAction.Invoke();
                    break;
            }
            ...
            void RemoveAll()
            {
                var (paths, outFailPaths) = ReturnVariantAssetsPath(assets);

                if (variantSetAsset != null)
                {
                    var path = AssetDatabase.GetAssetPath(variantSetAsset);
                    if (!string.IsNullOrEmpty(path))
                    {
                        paths.Add(AssetDatabase.GetAssetPath(variantSetAsset));
                        if (!outFailPaths.Contains(Directory.GetParent(AssetDatabase.GetAssetPath(variantSetAsset)).FullName))
                        {
                            outFailPaths.Add(Directory.GetParent(AssetDatabase.GetAssetPath(variantSetAsset)).FullName);
                        }
                    }
                }
                
                RemoveAssets(paths, outFailPaths);
            }
        }
        
        public static void RemoveAssetConfirmation(List<VariantAsset> assets)
        {
            DelayAction = RemoveAll;
            switch (PackageSettingsController.Settings.RemoveBehaviour)
            {
        ...
        {
            var result = EditorUtility.DisplayDialog("Remove Assets",
                "Do you want to remove the assets that were associated with this variant? (This is cannot be recovered.)" +
                "\nYou can modify this under settings", "Yes", "No");
            if (result)
            {
                DelayAction?.Invoke();
            }
